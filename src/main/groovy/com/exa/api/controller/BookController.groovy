...
class BookController{
   ...
   public ResponseEntity<Book> findById(@PathVariable('id') Integer id){
      ...
      if(bookService.findById(id) != null) {
         ...
         response.put("message", "buku tidak ditemukan")
   ...
   public ResponseEntity<Book> findByCategoryId(@PathVariable('id') Integer categoryId) {
       ...
       if(bookService.findByCategoryId(categoryId) != null) {
           ...
           response.put("status", "gagal")
           response.put("message", "buku tidsk ditemukan")
   ...
   public ResponseEntity<Book> createBook(@Valid @RequestBody Book book) {
      ...
      if(bookService.saveBook(book) != null) {
         ...
         response.put("message", "data berhasil dibuat")
         return ResponseEntity.ok(response)
      } else {
         response.put("status", "gagal")
         response.put("message", "data gagal dibuat")
   ...
   public ResponseEntity<Book> updateBook (@Valid @RequestBody Book book,
      @PathVariable(value= "id") Integer id) {
         ...
         if(bookService.updateBook(book, id) != null) {
            response.put("status", "berhasil")
            response.put("message", "data telah diubah")
            return ResponseEntity.ok(response)
         } else {
            response.put("status", "gagal")
            response.put("message", "buku tidak ditemukan")
